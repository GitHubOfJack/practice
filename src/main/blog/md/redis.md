 ...
 *              令牌桶： 以固定速度往桶里生产令牌，请求先获取令牌，获取成功，调用服务，获取失败，限流。 guava redis-cell也是令牌桶
 *              漏桶算法：流入速度不限制，桶满就溢出，靠流出速度控制，特点：稳定，缺点：面对突发请求无法及时处理
 ...
 *     秒杀的设计思路
 *     漏桶原理
 *     从客户端->CDN->前端系统->后台系统 ->DB口越来越小
 *     核心问题是：避免超卖问题、避免秒杀引起系统崩溃（秒杀系统必须单独服务单独部署、要有相应的限流、降级、熔断方案）
 *     可以在客户端缓存用户部分数据，同事客户端可以采用排队机制，例如：验证码输入等方式
 *     静态数据和动态数据分离CDN
 *     前后端异步处理MQ
 *     可以使用REDIS操作，后同步到数据库
 *     redis为什么性能比较高：
...
https://cloud.tencent.com/developer/news/607786    数据备份的原理

![redis-文件落盘方式](..\image\redis-文件落盘方式.png)

![redis-rdb流程](..\image\redis-rdb流程.png)

![redis-aop流程](..\image\redis-aop流程.png)